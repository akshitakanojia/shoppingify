...
const Footer = ({isEditState,setIsEditState}) => {
  ...
  return (
    ...
    {isEditState?<div className={styles.footer_wrapper}>
      <form className={styles.input_box} onSubmit={handleSave}>
        ...
        <button className={`${styles.btn} ${styles.save_btn}`} disabled={isCartEmpty}>Save</button>
      </form>
    </div>
    ...
    <button className={`${styles.btn} ${styles.form_save} ${styles.conflower_blue}`} onClick={()=>{dispatch(saveList('completed'));setIsEditState(false)}}>Complete</button>
  </div>}
    </>
  )
}
