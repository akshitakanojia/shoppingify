...
const ItemBadge = ({ item, quantity }) => {
  ...
  const { panelOpen } = useSelector(state => state.rightPanel)
  const added = !!cartItems?.find(cartItem => cartItem.item._id === item._id)

  return (
    <>
      <div className={styles.badge}>
        <div className={styles.item_name} onClick={() => { dispatch(changeState('item-view', item._id)); if (!panelOpen) dispatch(togglePanel()) }}>{item.name}</div>
        {
